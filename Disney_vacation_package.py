...
#function to assign cusotmer service rep when cusotmer requests a live agent
...
seeya = f"""
            ...
            / ___|  ___  ___  \ \ / /_ _  |  _ \ ___  __ _| | / ___|  ___   ___  _ __  
            \___ \ / _ \/ _ \  \ V / _` | | |_) / _ \/ _` | | \___ \ / _ \ / _ \| '_ \ 
             ___) |  __/  __/   | | (_| | |  _ <  __/ (_| | |  ___) | (_) | (_) | | | |
            |____/ \___|\___|   |_|\__,_| |_| \_\___|\__,_|_| |____/ \___/ \___/|_| |_|
...
print("""
         ...
         most amazing places in the world. Get a free quote for your
         WALT DISNEY WORLD vacation today! 
    
         Walk Disney World Vacations allows you to customize a vacation 
         to the Magic Kingdom in Florida. Take advantage of one of our  
         many packages that include your hotel, park tickets, and dining 
...
time.sleep(1)
print("\nHi " + name.title() + ". ", end="")
print("We're ready to begin building a MAGICAL vacation package!")
...
# Collects number of adult guests and validates. At least 1 adult rquired per reservation. 
while True:
      try:
        adults = int(input("\nHow many adults are traveling (10+ and older): "))
        if adults > 0:
          break
        print("\nEvery reservation must include at least (1) adult, please enter the number of adults in your party.")
      except ValueError:  
        print("\nOOOPS, INVALID ENTRY. Let's try again...")
...
# Collects number of rooms and validates input so it is a number greater than 0 and a valid entry (ie no spaces or dashes)
while True:
      try:
        number_of_rooms = int(input("\nHow many rooms would you like: "))
        if number_of_rooms > 0:
          break
        print("\nEvery reservation must include at least (1) room, please enter the number of rooms needed.")
      except ValueError:  
        print("\nOOOPS, INVALID ENTRY. Let's try again...")
...
clean_name = first_name.replace(" ", "") # remove spaces
valid = False
while not valid:
...
clean_last = last_name.replace(" ", "") # remove spaces
valid = False
while not valid:
    if clean_last.isalpha():
        valid = True
    else:
...
phone_clean = phone.replace(" ", "") # remove spaces
valid = False
while not valid:
    if phone_clean.isnumeric():
        valid = True
    else:
...
clean_address = street_address.replace(" ", "") # remove spaces
valid = False
while not valid:
    if clean_address.isalnum():
        valid = True
    else:
...
city = input("\nWhat is your city? ")
city_clean = city.replace(" ", "") # remove spaces
valid = False
while not valid:
    if city_clean.isalpha():
        valid = True
    else:
...
while not valid:
    if state.isalpha() and len(state) == 2:
        ...
        print("\nPlease enter a valid state 2 letter abbreviation.")
        state = input("\nWhat is your state? ")

# Collecting the user's zip code information using numeric validation
zip_code = input("\nWhat is your zip/postal code? ")
zip_code_clean = zip_code.replace(" ", "") # remove spaces
valid = False
while not valid:
    if zip_code_clean.isalnum():
        valid = True
    else:
...
country_clean = country.replace(" ", "") # remove spaces
valid = False
while not valid:
    if country_clean.isalpha():
        valid = True
    else:
...
time.sleep(3)
print("\nWe're ready to finalize your Walt Disney World Vacation Package. Please hold on a moment.")
...
if state == "FL":
    ...
    time.sleep(3)
    print(f'\nYour total Walt Disney World Vacation Package, including taxes, fees and your Florida discount is ${total_with_discount:.2f}.')  # ----- variable for grandtotal with Florida discount and 2 decimal places.
else:
    time.sleep(3)
    print(f'\nYour total Walt Disney World Vacation Package, including taxes and fees is ${total_wo_disc:.2f}.')  # -------- variable for Grandtotal without discount and 2 decimal places.

# Final confirmation message before payment information is collected
time.sleep(3)
print("\nPlease enter your payment information below to complete your Walt Disney World Vacation Package purchase.")
...
while not valid:
        if cc.isnumeric() and len(cc) == 6:
            ...
            print("\nPlease enter a valid credit card number, no dashes or spaces.")
            cc = input("\nWhat is your creidt card 6 digit number? ")
...
while not valid:
        if len(exp_date) == 4 and exp_date.isdigit():
            ...
            print("\nYour expiration date is not valid - make sure to use MMYY format, no dashes and two digits for each (example 0123 for Jan 2023)")
            exp_date = input("\nWhat is the expiration date of your card, using 4 numeric values MMYY? ")
 

# CVV and validation of length and numeric
cvv = input("\nWhat is the 3 digit CVV number?(Found on the back of your credit card) ")
valid = False
while not valid:
        if len(cvv) == 3 and cvv.isdigit():
            valid = True
        else:
            print("\nYour CVV number is not valid, please try again")
            cvv = input("\nWhat is the 3 digit CVV number? ")


# Payment confirmation message before the program ends  
time.sleep(3)    
print("\nThank you for your payment information, we are now processing your Walt Disney World Vacation Package. Please hold on a moment.")
...
time.sleep(3)
print("\nCONGRATULATIONS your Walt Disney World Vacation Package confirmation is", generate_reservation().upper(), "and will be sent to your email", email.lower(), "shortly.")

# Final message before the program ends
time.sleep(3)
print("\nYour all set - Thank you for choosing Walt Disney World Vacation Packages for your upcoming holiday.")
